...
for $i ( sort keys %h ){
	@list = @{$h{$i}};
	@list=@list[0,4,3,5,2,1];
	
	($f,$d) = fileparse($list[0]);
	$f=~/^(\w+?)_/;
	$QN="S$1";
	print "qsub -N GetMap /home/adminrig/src/short_read_assembly/bin/sub /home/adminrig/src/short_read_assembly/bin/GetMappingSeq.sh @list\nsleep 20"
}
