...
if [ -f "$1" ] & [ -f "$2" ];then
	...
	tblfilter -e ' $L{exp_freq_a1} != 0 && $L{exp_freq_a1} != 1 && $L{info} >= 0.4' $INFO | awk '{print $2}' | sed -n '2,$'p  > $RSID
	...
	/home/adminrig/src/qctool/qctool_v1.4-linux-x86_64/qctool -g $GEN -s $SAM -og $OUT -os $OUT.sample -incl-samples $TSAM -incl-rsids $RSID -omit-chromosome -sort &> $OUT.log
