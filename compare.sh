...
$sam = $intersection[0];
#print "$sam\n";
@Llist = sort keys %{$x{$sam}};
@Rlist = sort keys %{$y{$sam}};
...
print "\n\n";
print "intersect sample count :", @intersection+0,"\n";
print "sample @intersection\n" if @intersection < 1000;

print "\n\n";
print "intersect marker count :", @pos+0, "\n";
print "marker @pos\n" if @pos < 1000;
...
mmfss("Concordance.0.95.sample",%CR);
mmfss("Concordance",%Call);
mmfss("Concordance.sample",%Sampe);
mmfss("Concordance.mismatch", %mismatch);
}
' -- -x=$1 -y=$2 -cutoff=$3 -xcr=0.95 -ycr=0.95
#' -- -x=$1 -y=$2 
