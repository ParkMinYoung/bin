...
BEGIN{
$| = 1;
print join "\t", qw/marker sample geno x y/;
...
#				log_base(2,2)
				...
				$X = sprintf "%.5f", log_base($intensity{$m}{$_}{A}, 2) - log_base($intensity{$m}{$_}{B}, 2);
				$Y = sprintf "%.5f",( log_base($intensity{$m}{$_}{A}, 2) + log_base($intensity{$m}{$_}{B}, 2) ) / 2;
