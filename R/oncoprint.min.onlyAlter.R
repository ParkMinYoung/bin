...
setwd("/home/minmin/Dropbox/R/ComplexHeatmap/vignettes")
mat = read.table("OncoPrint.input.txt.transform.txt.only.gene.transform.txt", header = TRUE,stringsAsFactors=FALSE, sep = "\t")
#colnames(mat)
#mat[1,]
mat[is.na(mat)] = ""
rownames(mat) = mat[, 1]
...
add_oncoprint = function(type, x, y, width, height) {
  ...
  if(any(type %in% "FRAMESIFT")) {
    grid.rect(x, y, width - unit(0.5, "mm"), height - unit(1, "mm"), gp = gpar(col = NA, fill = type_col["FRAMESIFT"]))
  }
  if(any(type %in% "NONSENSE")) {
    grid.rect(x, y, width - unit(0.5, "mm"), height*(1/2), gp = gpar(col = NA, fill = type_col["NONSENSE"]))
  }
  if(any(type %in% "MISSENSE")) {
    grid.rect(x, y, width - unit(0.5, "mm"), height*(1/4), gp = gpar(col = NA, fill = type_col["MISSENSE"]))
  }
}
...
ha_column_bar = HeatmapAnnotation(df = data.frame(Group = c(rep("Normal vs. Tumor", 6), rep("Tumor vs. PDX", 5))),
