...
perl -F'\t' -anle'
($meth, $width, $median_meth_group1, $median_meth_group2, $median_meth_diff, @anno)= split ";", $F[3];
$k=join "\t",@F[0..2], $meth, $width, $median_meth_group1, $median_meth_group2, $median_meth_diff;


if(/# Chromo/){
    print join "\t", @F[0..2], qw/NumOfMethSites Width median.meth.group1 median.meth.group2 median.meth.diff/, "Region", "DetailRegion / Interval Betwen Genes / Codon change / distance to transcript", "transcript ID", "ExonTypeBefore", "Gene", "ExonTypeAfter", "Score";
}elsif( @anno ){
	for $i ( @anno ){
		$i =~ s/\|/\t/g;
		if( $i =~ s/Intergenic/Intergenic\t/ ){
...
cut -f9  $1.tab | sort | uniq -c | grep -v Effect | awk '{print $2"\t"$1}' >  $1.tab.EffectCount
#cut -f1-12 $1.tab > $1.tab.txt
cut -f1-11,13  $1.tab > $1.tab.txt
