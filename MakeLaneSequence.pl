...
Usage 
$0 --pair1 s_1_1_0001_qseq.txt --pair2 s_1_3_0001_qseq.txt --pair-index s_1_2_0001_qseq.txt [ --output-dir ./ ]
...
my ( $in_name, $in_dir ) = fileparse $index;
...
my $F3_file_name = $index;    # output file name
open my $F3, '<', $F3_file_name
  or die "$0 : failed to open  output file '$F3_file_name' : $!\n";
...
while (<$F1>) {
    ...
    my $mid = substr $in[8], 0, 6;
    my $line = $p1[0]."_". join( ":", $run_id, @p1[ 2 .. 5 ] ). "#$mid";
...
close $F3
  or warn "$0 : failed to close output file '$F3_file_name' : $!\n";
