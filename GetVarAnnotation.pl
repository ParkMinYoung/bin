...
#my $coord_file = "/home/adminrig/Genome/RefFlat/v37/refFlat.txt.AGL";
...
while (<$F>) {
    ...
    if ( defined $obj->{bin}{$chr}{$e_bin} ) {
        ...
        my $pos = Bio::Location::Simple->new( -start => $e, -end => $e );
        for (@refs) {
            ...
            if ( defined $res->{_match} ) {
                ...
                if ( $region =~ /CDS/ ) {
                    ...
                    my $codon_nt = substr $seq, 3 * ( $aa_pos - 1 ), 3;
                  ...
                  #print "$codon_nt/$codon_nt_mut\t$ori_aa\t$mut_aa\t$aa_pos\n";
                    print $W
"$line\t$gene\t$ref\t$region\t$ref_c\t$var_c\t$codon_nt\t$codon_nt_mut\t$ori_aa\t$mut_aa\t$ori_aa$aa_pos$mut_aa\t$codon\n";
                }
                else {
                    print $W "$line\t$gene\t$ref\t$region\n";
                }
                $found++;
            }
        }
...
#$all_region{$chr}{$ref} = $ref_region;
        ...
        map { $self->{bin}{$chr}{$_}{$id}++ }
          $gene_span_s_bin .. $gene_span_e_bin;
        print "read gene : $.\r";
    }
    close $F
      or warn "$0 : failed to close input file '$F_file' : $!\n";
