...
#perl -F'\t' -MMin -ane'$h{$F[7]}{$F[0]}++ }{ mmfsn("tile.depth.dist",%h)'  $1.bam.sorted.bam.pileup.tile
 ...
 #samtools view $1.bam.sorted.bam | perl -F'\t' -anle'print join "\t", $F[2],$F[3],$F[3]+length($F[9])-1 if @F > 10' > $1.bam.sorted.bam.read
...
mkdir -p $OUTDIR/{PE,SE,ALL,TrimDist}
mv $1.bam* $OUTDIR/ALL
...
#mv tile.depth.dist.txt $OUTDIR/Tile
