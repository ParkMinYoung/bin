...
if [ $# -ge 2 ];then
	...
	REF=/home/adminrig/Genome/Cow/bosTau7/bosTau7.fa
...
#sickle pe -f $R1T -r $R2T -q $Q -t illumina -l $L -o $R1_trim -p $R2_trim -s $R1_single >& $1_trim.sicle.log 
	...
	samtools sort -m 1000000000 $R1.bam $R1.bam.sort
	samtools index $R1.bam.sort.bam
	MarkDuplicates.Mark.sh $R1.bam.sort.bam
	samtools flagstat $R1.bam.sort.bam > $R1.bam.sort.bam.flagstats

	perl -sle'$f=~/(.+)?\/(.+)_\w{6,8}_L00/;print "AddOrReplaceReadGroups.sh $f $2"' -- -f=$R1.bam.sort.bam.Dedupping.Mark.bam | sh

	GenomeAnalysisTK.DepthOfCoverage.bosTau7.sh  $R1.bam.sort.bam.Dedupping.Mark.bam.AddRG.bam
