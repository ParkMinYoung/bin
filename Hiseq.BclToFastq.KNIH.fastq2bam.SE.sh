...
if   [ $# -eq 0 ];then
	...
	SHEET=$2
 	...
 	$Script --input-dir $PWD --intensities-dir $PWD/../ --positions-format .clocs --fastq-cluster-count 900000000 --sample-sheet $SHEET --output-dir $DIR --use-bases-mask Y101,I6n,n* >& $BCL2Fastq.log
	...
	waiting IsFilter
 	...
 	seq.stat.sh `find Project_* | grep fastq.gz$ | sort` 
  	...
  	batch.SGE.sh GATK.Pipeline.2.SE.sh `find Project_KNIH.set* | grep N.fastq.gz$ | sort` | perl -nle'if(/^qsub/){/Sample_(.+?)\//;$_="$_ $1 1"}print' > 01.Alignment
