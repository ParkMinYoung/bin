...
# Quality Score minimum mean value
IQ=$5
Qscore=${IQ:=20}
...
open $P2,">$p2.".$q."Trim";
if($singleF){
	open $S1,">$p1.".$q."Trim.single"; 
	open $S2,">$p2.".$q."Trim.single";
}		
}
push @p1, $F[0];
push @p2, $F[1];
if($.%4==0){
	...
	#print "@p1q\t@p2q";
	$p1_idx=firstidx { $_ < $q } @p1q;
	$p2_idx=firstidx { $_ < $q } @p2q;
...
' -- -p1=$1 -p2=$2 -len=$LEN -singleF=$WSRF -q=$Qscore > $1.${Qscore}Trim.log
else
	usage "fastq1 fastq2 [length:31] [singleReadWrite:0|1] [Qscore:20]"
fi
