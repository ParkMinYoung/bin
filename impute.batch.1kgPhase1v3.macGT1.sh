...
$hap = "$ref_dir/ALL_1000G_phase1integrated_v3_${chr}_impute_macGT1.hap.gz";
$legend = "$ref_dir/ALL_1000G_phase1integrated_v3_${chr}_impute_macGT1.legend.gz";
$known_hap = "$shapeit/${chr}_phased.haps";
$out = "$out_dir/${chr}_${start}_${end}";
print "$impute -use_prephased_g -m $map -h $hap -l $legend -known_haps_g $known_hap -int $start $end buffer 1000 Ne 14269 -o_gz -o $out -filt_rules_l \"asn.maf==0\" -allow_large_regions"
