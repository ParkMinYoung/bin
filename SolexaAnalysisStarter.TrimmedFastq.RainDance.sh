		...
		find s_? -maxdepth 2 | perl -nle'if(s/\.[12]$//){$h{$_}++} }{ map {/(s_\d)/;print "qsub -N PE$1 ~/src/short_read_assembly/bin/sub bwa.auto.PairedEndV2.S32M2Hg18.sh $_ 32 2\nsleep 10" } sort keys %h'  > PE.sh

        sh PE.sh

        # work for single read
         #make single folder
         find s_? -type f | grep single$ | xargs -i dirname {} | xargs -i mkdir -p {}/single
         #make symbolic link
         ...
         find s_? | grep "single/" | perl -nle'/(s_\d)/; print "qsub -N SE$1 ~/src/short_read_assembly/bin/sub bwa.auto.SingleEndV2.S32M2Hg18.sh $_ 32 2\nsleep 10" ' > SE.sh
         sh SE.sh
        ...
        find s_? -type f -name "*.bam" | grep -v sorted | perl -nle'/(s_\d+)/;push @{$h{$1}}, $_ }{ map{print "samtools merge $_/$_.bam @{$h{$_}}";`samtools merge $_/$_.bam @{$h{$_}};samtools rmdup -S $_/$_.bam $_/$_.rmdup.bam; mv $_/$_.bam $_/$_.bam.bak; mv $_/$_.rmdup.bam $_/$_.bam` } keys %h'
		...
		#find s_{6,7} | perl -nle'print "qsub -N $2 ~/src/short_read_assembly/bin/sub ./Bam2Tile.TrimedFastq.EZExome.sh $1" if /((s_\d)\/s_\d)\.bam$/' > bam2tile.sh
		#sh bam2tile.sh
